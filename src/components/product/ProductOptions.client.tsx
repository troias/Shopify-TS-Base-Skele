            ...
            {values.map((value) => {
                ...
                return (
                    <Text as="label" key={id} htmlFor={id}>
                        ...
                        />
                        <div
                            className={`leading-none py-1 border-b-[1.5px] cursor-pointer transition-all duration-200 ${checked ? 'border-primary/50' : 'border-primary/0'
                                }`}
                        >
                            {value}
                        </div>
            ...
            <Listbox onChange={updateSelectedOption} value="">
                ...
                {({ open }) => {
                    ...
                    return (
                        <>
                            <Listbox.Button
                                className={`flex items-center justify-between w-full py-3 px-4 border border-primary ${open ? 'rounded-b md:rounded-t md:rounded-b-none' : 'rounded'
                                    }`}
                            >
                ...
                duration-150 sm:bottom-auto md:rounded-b md:rounded-t-none md:border-t-0 md:border-b ${listboxOpen ? 'max-h-48' : 'max-h-0'
                                    }`}
                            >
                                {values.map((value) => {
                                    ...
                                    return (
                                        <Listbox.Option key={id} value={value}>
                                            ...
                                            {({ active }) => (
                                                <div
                                                    className={`text-primary w-full p-2 transition rounded flex justify-start items-center text-left cursor-pointer ${active ? 'bg-primary/10' : null
                                                        }`}
                                                >
                                                    {value}
                                                    {isSelected ? (
